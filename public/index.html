export async function onRequestPost(context) {
    ...
    if (action === 'getMissStats') {
        ...
        return new Response(JSON.stringify({ today: logs.filter(l => l.time >= startOfDay).length, month: logs.filter(l => l.time >= startOfMonth).length, year: logs.filter(l => l.time >= startOfYear).length }));
    }

    if (action === 'addWish') {
        const wishes = JSON.parse(await KV.get("birthday_wishes") || "[]");
        wishes.push({ id: Date.now(), content: content, time: new Date().toLocaleString(), year: new Date().getFullYear() });
        await KV.put("birthday_wishes", JSON.stringify(wishes));
        return new Response(JSON.stringify({ status: "ok" }));
    }

    if (action === 'getWishes') { return new Response(await KV.get("birthday_wishes") || "[]"); }
    if (action === 'delWish') {
